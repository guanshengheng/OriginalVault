...
## 总结和展望
本项目实现了一个基于调用大语言模型的AI可联网搜索的agent，通过mcp协议扩展了模型的能力边界。不再是只能自然语言对话，还能做到搜索实时数据。
回顾起来感觉自己的这个项目确实很基础，但是实现起来还是多多少少遇到了一些问题，不过通过各种方式进行了解决。 
后端没有分离解耦，是面向过程而非面向对象。没有用到数据库进行数据持久化。然后还有逻辑上可优化的点，比如没有记录日志、没有异常处理、没有实现真正的多段的对话等等。
前端界面也需要优化，部署上打算以后回头了解了解nginx等的相关内容。