代码托管在Github上，地址是[task2](https://github.com/guanshengheng/task2)

## 安装和配置 Ollama 工具
在官网链接进行下载
![](assets/Task2/file-20260106180359450.png)


## 拉取并运行 LLM 模型
在命令行拉取并运行
![](assets/Task2/file-20260106180518224.png)
![](assets/Task2/file-20260106180705395.png)

## 使用第三方工具连接到Ollama 并调用 LLM 模型进行对话
使用了chatbox进行连接，调用llm模型对话
![](assets/Task2/file-20260106180908804.png)
![](assets/Task2/file-20260106180835321.png)

## 使用python程序调用拉取的LLM模型对话
创建Client客户端对象。无限循环从而持续对话直到用户终止循环。对用户的输入构造请求，对模型返回解析输出。
![](assets/Task2/file-20260106181209927.png)
![](assets/Task2/file-20260106181158643.png)