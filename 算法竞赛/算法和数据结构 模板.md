...
void mfy(int jobl, int jobr, int jobv, int l, int r, int i)
{
    ...
    else
    {
        ...
        if (jobl <= mid) mfy(jobl, jobr, jobv, l, mid, i * 2);
        if (jobr > mid) mfy(jobl, jobr, jobv, mid + 1, r, i * 2 + 1);
        up(i);
    }
}
...
int que(int jobl, int jobr, int l, int r, int i)
{
    ...
    down(i, mid - l + 1, r - mid);
    int ans = 0;
    if (jobl <= mid) ans += que(jobl, jobr, l, mid, i * 2);
    if (jobr > mid) ans += que(jobl, jobr, mid + 1, r, i * 2 + 1);
    return ans;
}
```
...
void mfy(int l, int r, int i, int pos, int v)
{
    ...
    int mid = (l + r) / 2;
    if (pos <= mid) mfy(l, mid, i * 2, pos, v);
    else mfy(mid + 1, r, i * 2 + 1, pos, v);
    up(i);
}
...
int que(int jobl, int jobr, int l, int r, int i)
{
    ...
    int ans = 0;
    if (jobl <= mid) ans += que(jobl, jobr, l, mid, i * 2);
    if (jobr >= mid + 1) ans += que(jobl, jobr, mid + 1, r, i * 2 + 1);
    return ans;
}
